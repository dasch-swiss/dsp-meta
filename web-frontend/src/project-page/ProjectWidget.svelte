...
{#if $projectMetadata}
  ...
  <div class="widget">
      ...
      <span style="display:inline">
        Permalink
          <button on:click={copyPermalink} title="copy permalink to the clipboard">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path
      ...
      {#each $projectMetadata?.project.disciplines as d}
        ...
        {:else}
          {#if getText(d).match(/^\d+ /)}
            <a class="data" href=http://www.snf.ch/SiteCollectionDocuments/allg_disziplinenliste.pdf
    ...
    {#if $projectMetadata?.project.keywords}
      ...
      <span class="keyword">{$projectMetadata?.project.keywords.map(t => {
        return getText(t)
      }).join(", ")}</span>
    {:else if $isTestEnvironment}
      <div class=label>Keywords</div>
