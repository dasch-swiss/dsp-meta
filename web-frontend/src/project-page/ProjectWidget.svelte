...
{#if $projectMetadata}
  ...
  <div class="widget">
    ...
    <div class="data">
      {$projectMetadata?.project.dataManagementPlan
        ? 'available'
        : 'unavailable'}
    </div>
    ...
    <!-- Funders -->
    {#if $projectMetadata?.project.funders && $projectMetadata?.project.funders
        .map((o) => {
    ...
    {#if $projectMetadata?.project.keywords}
      ...
      <span class="keyword"
        >{$projectMetadata?.project.keywords
          .map((t) => {
            return getText(t);
          })
          .join(', ')}</span
